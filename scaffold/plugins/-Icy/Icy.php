...
class Icy extends Plugins
{
	...
	{
		self::load_library('Icy_compressor');
	    ...
	    if (CSScaffold::config('short_hex', 'Icy'))
	    {
		    for ($n = 0; $n < count(Icy_compressor::$file_props); $n++)
		    {
	    ...
	    if (CSScaffold::config('short_margins_and_paddings'))
	    {
		    for ($n = 0; $n < count(Icy_compressor::$file_props); $n++)
		    {
		        # run all the individual functions to reduce their size
	    ...
	    if (CSScaffold::config('combine_identical_rules', 'Icy'))
	    {
