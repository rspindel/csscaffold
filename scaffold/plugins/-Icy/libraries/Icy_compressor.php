...
abstract class Icy_compressor
{

	public static $file_props;
	public static $file_selector;
	
	public static function compress_css()
	{
	    ...
	    if ($_REQUEST['opt_combine_props_list'])
	    {
	        ...
	        for ($n = 0; $n < count(self::$file_props); $n++)
	            // attempt to combine the different parts
	            combine_props_list(self::$file_props[$n]);    
	    }
	    
	    // for each rule in the file
	    for ($n = 0; $n < count(self::$file_props); $n++)
	        // run all the individual functions to reduce their size
	        array_walk(self::$file_props[$n], 'reduce_prop');
	    ...
	    if ($_REQUEST['opt_output_stats'])
	    {
	        ...
	        echo '<ul>';
	        echo '<li>Rules: ' . count(self::$file_selector) . '</li>';
	 
	        for ($n = 0; $n < count(self::$file_selector); $n++)
	            $selectors += count(self::$file_selector[$n]);
	        
	        for ($n = 0; $n < count(self::$file_props); $n++)
	            $props += count(self::$file_props[$n]);
	...
	public static function get_sent_css()
	{
	...
	public static function kill_comments(&$css)
	{
	    // kill html comments
	    $css = str_replace('<!--', '', $css);
	    $css = str_replace('-->', '', $css);
	...
	public static function rgb2hex(&$string)
	{
		$text = "";
	...
	public static function long_colours_to_short_hex(&$string)
	{
	...
	public static function long_hex_to_short_colours(&$string)
	{    
	...
	public static function remove_zero_measurements(&$string)
	{
	...
	public static function sort_css($cssfile)
	{
	...
	public static function font_weight_text_to_numbers(&$string)
	{
	    // make these variables available to this function
	    
	    
	    
	    for ($a = 0; $a < count(self::$file_props); $a++)
	    {
	        for ($b = 0; $b < count(self::$file_props[$a]); $b++)
	        {
	            if (self::$file_props[$a][$b] == 'font-weight:bold')
	                self::$file_props[$a][$b] = 'font-weight:700';
	            if (self::$file_props[$a][$b] == 'font-weight:normal')
	                self::$file_props[$a][$b] = 'font-weight:400';
	                
	            self::$file_props[$a][$b] = str_replace('font:normal', 'font:400', self::$file_props[$a][$b]);
	            self::$file_props[$a][$b] = str_replace('font:bold', 'font:700', self::$file_props[$a][$b]);    
	        }
	    }
	}
	 
	// seperate the parts of each rule
	public static function parse_rules($css)
	{
	    // make these variables available to this function
	...
	public static function combine_identical_selectors()
	{
	    ...
	    for ($a = 0; $a < count(self::$file_selector); $a++)
	    {
	        ...
	        if (count(self::$file_selector[$a]) == 1)
	        {
	            ...
	            if (in_array(self::$file_selector[$a][0], $cur_selectors))
	            {
	                ...
	                for ($b = 0; $b < count($cur_selectors); $b++)
	                {
	                    ...
	                    if ($cur_selectors[$b] == self::$file_selector[$a][0])
	                    {
	                        ...
	                        $new_props = array_merge(self::$file_props[$b], self::$file_props[$a]);
	                        // replace the current props with all of them
	                        self::$file_props[$a] = $new_props;
	                        // kill the old selector
	                        self::$file_selector[$b] = NULL;
	                        // kill the old properties
	                        self::$file_props[$b] = array(NULL);
	            ...
	            $cur_selectors[] = self::$file_selector[$a][0];        
	        }
	        else
	...
	public static function remove_overwritten_properties()
	{
	    // make these arrays available to this function
	    ...
	    for ($a = 0; $a < count(self::$file_props); $a++)
	    {
	        ...
	        for ($b = 0; $b < count(self::$file_props[$a]); $b++)
	        {
	            ...
	            $parts = explode(':', self::$file_props[$a][$b]);
	...
	public static function combine_props_list(&$props)
	{
	    ...
	    self::combine_props($props, 'padding', array('padding-top', 'padding-right', 'padding-bottom', 'padding-left'));    
	    
	    self::combine_props($props, 'margin', array('margin-top', 'margin-right', 'margin-bottom', 'margin-left'));    
	 
	    self::combine_props($props, 'list-style', array('list-style-type', 'list-style-position', 'list-style-image'));
	    self::combine_props($props, 'list-style', array('list-style-type', 'list-style-position'));
	    
	    self::combine_props($props, 'outline', array('outline-color', 'outline-style', 'outline-width'));    
	    
	    // to do: this might be improvable
	    self::combine_props($props, 'background', array('background-color', 'background-image', 'background-repeat', 'background-attachment', 'background-position'));
	 
	    // to do: combine all border-[places] if they're the same (need a special function for that)
	    self::combine_props($props, 'border-bottom', array('border-bottom-width', 'border-bottom-style', 'border-bottom-color'));
	    self::combine_props($props, 'border-top', array('border-top-width', 'border-top-style', 'border-top-color'));
	    self::combine_props($props, 'border-left', array('border-left-width', 'border-left-style', 'border-left-color'));
	    self::combine_props($props, 'border-right', array('border-right-width', 'border-right-style', 'border-right-color'));
	    
	    // to do: this needs some checking
	    self::combine_props($props, 'font', array('font-style', 'font-variant', 'font-weight', 'font-size', 'line-height', 'font-family'));
	    self::combine_props($props, 'font', array('font-style', 'font-variant', 'font-weight', 'font-size', 'font-family'));
	    self::combine_props($props, 'font', array('font-variant', 'font-weight', 'font-size', 'line-height', 'font-family'));
	    self::combine_props($props, 'font', array('font-style', 'font-weight', 'font-size', 'line-height', 'font-family'));
	    self::combine_props($props, 'font', array('font-style', 'font-variant', 'font-size', 'line-height', 'font-family'));
	    self::combine_props($props, 'font', array('font-variant', 'font-weight', 'font-size', 'font-family'));
	    self::combine_props($props, 'font', array('font-style', 'font-weight', 'font-size', 'font-family'));
	    self::combine_props($props, 'font', array('font-style', 'font-variant', 'font-size', 'font-family'));
	    self::combine_props($props, 'font', array('font-variant', 'font-size', 'font-family'));
	    self::combine_props($props, 'font', array('font-weight', 'font-size', 'font-family'));
	    self::combine_props($props, 'font', array('font-style', 'font-size', 'font-family'));    
	}
	...
	public static function combine_props(&$props, $combined, $parts)
	{
	...
	public static function reduce_prop(&$item, $key)
	{
	    ...
	    if ($_REQUEST['opt_short_margins_and_paddings']);
	        self::compress_padding_and_margins($item);
	}
	...
	//  #ff0000 -> #f00
	//  #aabbcc -> #abc
	public static function short_hex(&$item)
	{
	...
	public static function compress_padding_and_margins(&$item)
	{
	...
	public static function remove_empty_rules()
	{
	    // make these arrays available to this function
	    ...
	    for ($a = 0; $a < count(self::$file_selector); $a++)
	    {
	        ...
	        self::$file_props[$a] = array_values(array_diff(self::$file_props[$a], array(NULL)));
	        
	        // check if this part has no properties
	        if (!isset(self::$file_props[$a][0]))
	        {
	            // remove the empty prop part of the array and the class(es)
	            array_splice(self::$file_selector, $a, 1);
	            array_splice(self::$file_props, $a, 1);
	...
	public static function combine_identical_rules()
	{
	    // make these variables available locally
	    ...
	    for ($a = 0; $a < count(self::$file_props); $a++)
	    {
	        ...
	        for ($b = 0; $b < $a; $b++)
	        {
	            if (substr(self::$file_selector[$a][0], 0, 1) <> "@" && substr(self::$file_selector[$b][0], 0, 1) <> "@")
	            {
	                // check if this rule is identical to an earlier one
	                if (!array_diff(self::$file_props[$a], self::$file_props[$b]) && !array_diff(self::$file_props[$b], self::$file_props[$a]))
	                {
	                    // combine the selectors
	                    self::$file_selector[$a] = array_unique(array_merge(self::$file_selector[$a], self::$file_selector[$b]));
	                    // remove the old properties
	                    self::$file_props[$b] = array(NULL);
	                    // remove the old selectors
	                    self::$file_selector[$b] = array(NULL);
	                }
	            }
	        }
	    }
