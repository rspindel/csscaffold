...
class Scaffold_Controller
{
	...
	public static function find_file($filename, $directory = '', $required = FALSE)
	{
		...
		if ($found === NULL)
		{
			if ($required === TRUE)
			{
				...
				throw new Exception("Cannot find the file: " . str_replace($_SERVER['DOCUMENT_ROOT'], '', $directory . $filename));
			}
			else
			{
	...
	public static function add_include_path($path)
	{
		...
		{
			$args = func_get_args();
			self::$include_paths = array_merge($args, self::$include_paths);
		}
		else
		{
	...
	public static function cache_clear($path = "")
	{
		...
		{
			Scaffold_Utils::stop(dirname(basename($path)));
			unlink($path);
			return true;
		}
		elseif(is_dir($path))
	...
	public static function exception_handler($exception, $message = NULL, $file = NULL, $line = NULL)
	{
		try
		{
			...
			require self::config('core.path.system') . 'views/Exception.php';
