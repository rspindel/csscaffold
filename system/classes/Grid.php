...
{	
	...
	{	
		...
		{						
			$options['Grid']['columncount'] 	= 	$this -> getParam('column-count', $match[0][0]);
			$options['Grid']['columnwidth']	= 	$this -> getParam('column-width', $match[0][0]);
			$options['Grid']['gutterwidth']	= 	$this -> getParam('gutter-width', $match[0][0]);
			$options['Grid']['baseline']		=	$this -> getParam('baseline', $match[0][0]);
						
			// Check whether we should use the column width or calculate it from the grid width
			if ($options['Grid']['columnwidth'] == "") 
			{
				$options['Grid']['gridwidth']	= $this -> getParam('grid-width', $match[0][0]);
				$options['Grid']['columnwidth'] = $this -> getColumnWidth();
			}
			else 
			{
				$options['Grid']['columnwidth'] = $options['Grid']['columnwidth'] + $options['Grid']['gutterwidth'];
				$options['Grid']['gridwidth'] = ($options['Grid']['columnwidth'] * $options['Grid']['columncount']) - $options['Grid']['gutterwidth'];
			}	
			
			// If theres no format specified, go with 'newline'
			if ($options['Grid']['format'] == "") 
			{
				$options['Grid']['format'] = "newline";
			}		
		}
	...
	public function generateGridClasses($css)
	{
		...
		if($options['Grid']['columns-x'] == TRUE)
		{
			// Make the .columns-x classes
			for ($i=1; $i < $options['Grid']['columncount'] + 1; $i++) { 
				$w = $options['Grid']['columnwidth'] * $i - $options['Grid']['gutterwidth'];
				$s .= ".columns-$i{ width:".$w."px; }";
			}
		}
	
		if($options['Grid']['push'] == TRUE)
		{
			// Make the .push classes
			for ($i=1; $i < $options['Grid']['columncount']; $i++) { 
				$w = $options['Grid']['columnwidth'] * $i;
		...
		if($options['Grid']['baseline-x'] == TRUE)
		{
			// Make the .baseline-x classes
			for ($i=1; $i < 51; $i++) { 
				$h = $options['Grid']['baseline'] * $i;
				$s .= ".baseline-$i{ height:".$h."px; }";
			}
		}
		
		if($options['Grid']['baseline-pull-x'] == TRUE)
		{
		...
		if($options['Grid']['baseline-push-x'] == TRUE)
		{
	...
	public function generateGridImage($css)
	{
		...
		$image = ImageCreate($options['Grid']['columnwidth'], $options['Grid']['gutterwidth']);
		...
		// Draw column
		Imagefilledrectangle($image, 0, 0, ($options['Grid']['columnwidth'] - $options['Grid']['gutterwidth'] - 1), ($options['Grid']['baseline'] - 1), $colorBlue);
		
		// Draw gutter
		Imagefilledrectangle($image, ($options['Grid']['columnwidth'] - $options['Grid']['gutter'] + 1), 0, ($options['Grid']['columnwidth']), ($options['Grid']['baseline'] - 1), $colorWhite);
	
		// Draw baseline
		imageline($image, 0, ($options['Grid']['baseline'] - 1 ), $options['Grid']['columnwidth'], ($options['Grid']['baseline'] - 1 ), $colorGrey);
		...
		for ($i=1; $i <= $options['Grid']['columncount']; $i++) { 
			...
			while (preg_match_all('/\{([^\}]*(columns\:\s*('.$i.'!?)\s*\;).*?)\}/sx', $css, $match)) {
				...
				foreach ($match[0] as $key => $properties)
				{
					...
					$width = (($options['Grid']['columnwidth']*$i)-$options['Grid']['gutterwidth']);
					...
					if ($showproperties) 
					{
						...
						if ($numberofcolumns < $options['Grid']['columncount'])
						{
							$styles .= "margin-right:" . $options['Grid']['gutterwidth'] . "px;";
						}
					}
	...
	{	
		...
		if (preg_match_all('/grid\((\d+)?col\)/', $css, $matches))
		{
			...
			{
				$colw = ($number * $options['Grid']['columnwidth']) - $options['Grid']['gutterwidth'] .'px';		
				$css = str_replace($matches[0][$key],$colw,$css);
			}
		}
		
		// Replace grid(max)
		$maxw = ($options['Grid']['columncount'] * $options['Grid']['columnwidth']) - $options['Grid']['gutterwidth'] .'px';
		$css = str_replace('grid(max)',$maxw,$css);
		
		// Replace grid(baseline)
		$bl = $options['Grid']['baseline'].'px';		
		$css = str_replace('grid(baseline)', $bl, $css);
		
		// Replace grid(gutter)
		$gutter = $options['Grid']['gutterwidth'].'px';		
	...
	private function getColumnWidth() 
	{
		...
		$grossgridwidth		= $options['Grid']['gridwidth'] - ($options['Grid']['gutterwidth'] * ($options['Grid']['columncount']-1)); /* Width without gutters */
		$singlecolumnwidth 	= $grossgridwidth/$options['Grid']['columncount'];
		$columnwidth 		= $singlecolumnwidth + $options['Grid']['gutterwidth'];
