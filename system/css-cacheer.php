...
$ext_path = $system_dir."/extensions";

if (is_dir($ext_path))
{
	if ($dir_handle = opendir($ext_path)) 
	{
		while (($ext_file = readdir($dir_handle)) !== false) 
		{
			if (substr($ext_file, 0, 1) == '.' || substr($ext_file, 0, 1) == '-')
			{ 
				continue; 
			}
			
			include($ext_path.'/'.$ext_file);
